...
export const fetchStories = async (): Promise<Story[]> => {
  try {
    const response = await axios.get<StoriesResponse>(`${API_BASE_URL}/stories`);
...
export const fetchStoryById = async (storyId: string): Promise<Story> => {
  try {
    const response = await axios.get<StoryResponse>(`${API_BASE_URL}/stories/${storyId}`);
    if (response.data.success && response.data.data) {
      return response.data.data;
    }
...
export const fetchQuizByStoryId = async (storyId: string): Promise<Quiz> => {
  try {
    const response = await axios.get<QuizResponse>(`${API_BASE_URL}/quiz/${storyId}`);
    if (response.data.success && response.data.data) {
      return response.data.data;
    }
...
export const submitQuiz = async (storyId: string, answers: number[]): Promise<QuizResult> => {
  try {
